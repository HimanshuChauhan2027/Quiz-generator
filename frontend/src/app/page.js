        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            ...
            },
            {
              ...
              title: 'AI Generates',
              desc: 'AI crafts unique, engaging questions perfectly tailored to your topic.',
            },
            {
              step: '03',
              icon: (
...
export default function Home({ initialQuiz = null }) {
  ...
  // Scroll to the auth section once the page has finished loading, so links like
  // "/#auth-section" (e.g. the "Sign In" nav button) work even when the target
  // element wasn't in the DOM yet at initial page load.
  useEffect(() => {
    if (loading || isAuthenticated) return;
    if (typeof window === 'undefined' || window.location.hash !== '#auth-section') return;
    const el = document.getElementById('auth-section');
    el?.scrollIntoView({ behavior: 'smooth' });
